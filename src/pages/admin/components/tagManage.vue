...
const refreshData = (obj) => {
  if (obj.isAdd) {
    ...
    updateTag({ id: tagId.value, name: obj.name }).then((res) => {
      if (res.success) {
        successMsg('文章标签更新成功！')
        tagId.value = null
        tagDialog.value.dialogVisible = false
...
function handleDeleteOpration() {
  ElMessageBox.confirm('是否删除当前选择的文章标签?', '文章标签删除', {
    ...
    .then(() => {
      deleteTag({ idList: idList.value }).then((res) => {
        if (res.success) {
          successMsg('删除成功！')
    ...
    <el-card shadow="never" class="my-1 shadow-sm">
      ...
      <div class="flex items-center">
        ...
        <div class="ml-3 w-52 mr-5">
          <el-input v-model="searchTagName" placeholder="请输入文章类别名称" />
        </div>
