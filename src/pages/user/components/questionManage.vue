...
import {
  deleteQuestionByIdLogical,
  getPersonalQuestionList
} from '@/api/user/ask'
import Preview from '@/comps/Preview.vue'
import { ElMessage, ElMessageBox } from 'element-plus'

const questionSize = ref(0)
const reqForm = reactive({ key: '', current: 1, size: 10 })
const questionPageData = ref()
const isLoaded = ref(false)
const dialogVisible = ref(false)
const isLockLoadMore = ref(false)
onMounted(() => {
  getPersonalQuestionList(reqForm).then((res) => {
    if (res.success) {
      questionPageData.value = res.data
      isLoaded.value = true
      questionSize.value = res.data.length
    }
  })
})
const searchQuestionByKey = () => {
  reqForm.current = 1
  isLockLoadMore.value = false
  getPersonalQuestionList(reqForm).then((res) => {
    if (res.success) {
      questionPageData.value = res.data
      isLoaded.value = true
...
const showDetail = (index: string | number) => {
  console.log(questionPageData.value[index])

  dialogVisible.value = true
  mkdContent.value = questionPageData.value[index].content
}

const deleteQuestion = (index: any) => {
  ElMessageBox.confirm('确定删除该问答吗？删除后将不可恢复!', '消息提示', {
    ...
    .then(() => {
      const id = questionPageData.value[index].id
      deleteQuestionByIdLogical(id).then((res) => {
        if (res.success) {
          ElMessage({
            type: 'success',
            message: '删除成功！'
          })
          questionPageData.value.splice(index, 1)
        }
      })
    })
    .catch(() => {
...
const loadMoreQuestion = () => {
  ...
  getPersonalQuestionList(reqForm).then((res) => {
    if (res.success) {
      ...
      questionPageData.value.push(...res.data)
      isLoaded.value = true
      questionSize.value = res.data.length
    }
  })
...
<template>
  <el-dialog v-model="dialogVisible" center align-center title="问答详情">
    <Preview :content="mkdContent"></Preview>
  </el-dialog>

  <div class="w-full border-b h-[55px] p-3 flex justify-between items-center">
    <div class="flex items-center gap-5">
      <div class="cursor-pointer">
        我的问答(<span class="text-blue-600">{{ questionSize }}</span
        >)
      </div>
    </div>
    <div>
      <el-input
        ...
        :suffix-icon="Search"
        @change="searchQuestionByKey"
      />
    </div>
  </div>
  <div
    class="Container box-border min-h-[570px] max-h-[570px] overflow-auto p-2"
  >
    <div
      v-infinite-scroll="loadMoreQuestion"
      :infinite-scroll-disabled="isLockLoadMore"
      :infinite-scroll-immediate="false"
      v-if="isLoaded && questionPageData.length !== 0"
    >
      <!-- 文章item -->
      <div
        v-for="(question, index) in questionPageData"
        :key="question.id"
        class="select-none flex p-1 shadow-sm hover:bg-[#F7F8FA] cursor-pointer"
      >
        <div class="flex-col flex-1">
          <div @click="showDetail(index)" class="p-3">
            <p
              v-if="question.editorType === 0"
              class="line-clamp-2 text-[#606266] text-sm"
            >
              {{ question.content }}
            </p>
            <p v-else class="text-[#909399] text-sm">
              MarkDown类型问答消息，点击查看问答详情
            </p>
          </div>
              ...
              <el-popover trigger="hover">
                ...
                <template #default>
                  <div class="flex justify-center">
                    <el-button @click="showDetail(index)" type="info"
                      >详情</el-button
                    >
                    <el-button @click="deleteQuestion(index)" type="danger"
                      >删除</el-button
                    >
                  </div>
                </template>
